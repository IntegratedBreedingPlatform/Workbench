...
public class SchemaVersionUtilTest {
	...
	public void setup() throws SecurityException, NoSuchFieldException, Exception {
		this.workbenchFile = new File(this.workbenchFilePath);
		FileWriter workbenchFileWriter = new FileWriter(this.workbenchFile);
		this.minimumCropVersion = "3.0.3";
		String fileContent = SchemaVersionUtil.MINIMUM_CROP_VERSION + "=" + this.minimumCropVersion;
		PrintWriter printWriter = new PrintWriter(workbenchFileWriter);
		printWriter.print(fileContent);
		printWriter.close();
		workbenchFileWriter.close();

		this.emptyFile = new File(this.emptyFilePath);
		FileWriter emptyFileWriter = new FileWriter(this.emptyFile);
	...
	public void testNullOrEmptyMinimumCropVersion() {
		...
		Assert.assertTrue("Version can be null if minimum crop version is null", SchemaVersionUtil.checkIfVersionIsSupported(null, null));
		Assert.assertTrue("Version can be null if minimum crop version is empty", SchemaVersionUtil.checkIfVersionIsSupported(null, ""));
	}
	...
	public void testLoadPropertiesNonExistingFile() {
		Assert.assertNull("A non-existing properties file should return null", SchemaVersionUtil.loadPropertiesFile("dummy.properties"));
	}

	@Test
	public void testLoadPropertiesExistingFile() {
		Assert.assertNotNull("An existing properties file should not return null",
				SchemaVersionUtil.loadPropertiesFile(this.workbenchFilePath));
	}
	...
	public void testGetMinimumGetVersionNotFound() {
		Assert.assertEquals("Minimum crop version is empty if it is not defined in the config file", "",
				SchemaVersionUtil.getMinimumCropVersionFromFile(this.emptyFilePath));
	}

	@Test
	public void testGetMinimumGetVersionFound() {
		String expectedCurrentCropVersion = this.minimumCropVersion;
		String actualCurrentCropVersion = SchemaVersionUtil.getMinimumCropVersionFromFile(this.workbenchFilePath);
		Assert.assertNotNull("Minimum crop version is not null if it is defined in the config file", actualCurrentCropVersion);
		Assert.assertEquals("Minimum crop version must be equal to " + expectedCurrentCropVersion, expectedCurrentCropVersion,
				actualCurrentCropVersion);
	}

	@After
	public void cleanUp() {
		if (this.workbenchFile != null) {
			this.workbenchFile.delete();
		}
