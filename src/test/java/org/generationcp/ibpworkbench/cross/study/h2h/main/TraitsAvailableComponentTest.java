...
public class TraitsAvailableComponentTest {
	...
	public void setup() {
		...
		Mockito.doReturn(TRAITS).when(this.messageSource).getMessage(Message.HEAD_TO_HEAD_CHECK_TRAITS);
		Mockito.doReturn(ANALYSIS).when(this.messageSource).getMessage(Message.HEAD_TO_HEAD_CHECK_ANALYSIS);
		...
		Mockito.doReturn(this.pairs).when(this.crossStudyDataManager).getEnvironmentsForGermplasmPairs(Mockito.anyListOf(GermplasmPair.class), Mockito.anyListOf(Integer.class));
	}
	...
	public void testRefreshEnviromentPairListWhenAllVariablesSelected() {
		...
		Mockito.verify(this.crossStudyDataManager).getEnvironmentsForGermplasmPairs(Mockito.eq(this.pairs), Mockito.eq(experimentTypes));
	}
	...
	public void testRefreshEnviromentPairListWhenTraitsSelected() throws Exception {
		...
		this.traitsAvailableComponent.getVariableFilterOptionGroup().setValue(TRAITS);
	...
	public void testRefreshEnviromentPairListWhenAnalysisVariablesSelected() throws Exception {
		this.traitsAvailableComponent.afterPropertiesSet();
		this.traitsAvailableComponent.getVariableFilterOptionGroup().setValue(ANALYSIS);
