function GraphicalFilter() {
	...
	Filter.operator_map = d3.map(Filter.operators, function (d) {
		return d.name
	});
	...
	FilterRange.prototype.drawHistogram = function () {
		...
		this.y.domain([0, d3.max(bins, function (d) {
			return d.length;
		})]);

		//order bins to be ID'd by distance to the centerVal. (sorted by *signed* distance)
		var cvBisector = d3.bisector(function (bin) {
			return d3.mean([bin.x0, bin.x1]);
		}).left;
		...
		entered.append("rect")
			.attr("fill", "#c9c")
			.attr("y", function (d) {
				return hist.y(d.length)
			})
			.attr("height", function (d) {
				return bar_height - hist.y(d.length);
			})
			.attr("width", 0)
			.attr("x", this.b_width)
			.filter(function (d) {
				return d.keyID < hist.centerID
			})
			.attr("x", 0);
		entered.append("text")
		...
		newBars.select("rect")
			...
			.attr("x", function (d) {
				return hist.x(d.x0) + 1
			});
		newBars.select("text")
			.transition().duration(this.ttime)
			.attr("opacity", 1)
			.attr("x", function (d) {
				return hist.x((d.x0 + d.x1) / 2)
			})
			.attr("y", function (d) {
				return hist.y(d.length) - 2
			})
