...
public class HeadToHeadResultsUtil {
	...
	public static double getPvalue(int numOfEnvts, int numOfSucesses) {
		BinomialDistribution binomial = new BinomialDistribution(numOfEnvts, HeadToHeadResultsUtil.DEF_PROBABILITY);
	...
	public static Double getMeanDiff(GermplasmPair germplasmPair, TraitForComparison traitForComparison,
		...
		/*
		 * Nijl = is the number of environment where both tijk and silk is not null and not empty string r = 1 if increasing and -1 if
		 * decreasing Ek - environment weight
		 */
		...
		while (diffIterator.hasNext() && envtIterator.hasNext() && totalWeight != 0) {
			...
			double envtWeight = envtIterator.next();
	...
	public static Integer getTotalNumOfSup(GermplasmPair germplasmPair, TraitForComparison traitForComparison,
			Map<String, ObservationList> observationMap, List<EnvironmentForComparison> environmentForComparisonList) {
		boolean isIncreasing = false;
		int counter = 0;
		if (traitForComparison.getDirection().intValue() == TraitsAvailableComponent.INCREASING.intValue()) {
	...
	public static Integer getTotalNumOfEnv(GermplasmPair germplasmPair, TraitForComparison traitForComparison,
			Map<String, ObservationList> observationMap, List<EnvironmentForComparison> environmentForComparisonList) {
		int counter = 0;
	...
	/*
	 * Gets mean of all observed values for a trait for a germplasm in the pair. Mean = sum of observed values for germplasm / # of envt's
