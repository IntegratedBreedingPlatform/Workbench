...
public class TableViewerCellSelectorUtil {
	 ...
	 * @param sourceTable
	 */
	public TableViewerCellSelectorUtil(final Object source, final Table sourceTable) {
		this.source = source;
	...
	public TableViewerCellSelectorUtil(final Table sourceTable) {
		this.table = sourceTable;
		this.columnHeaders = new ArrayList<String>();
		this.highlightedCellCoordinates = new ArrayList<CellCoordinate>();
		this.customCSSClassNames = new ArrayList<String>();
	...
	private void initialize() {
		...
		if (this.source instanceof Window) {
			((Window) this.source).addComponent(new Label(
			...
			((AbstractOrderedLayout) this.source).addComponent(new Label(
					"<style> .v-table .v-table-cell-content-highlighted { background: #1B91E0; padding-bottom: -1px; border-bottom: 1px solid #146296; padding-right: -1px; border-right: 1px solid #146296; } </style>",
					Label.CONTENT_XHTML));
			((AbstractOrderedLayout) this.source).addComponent(new Label(
					"<style> .v-table .v-table-cell-content-currentlyselected { background: #1B91E0; padding: -2px; border: 2px solid #3B25B3; } </style>",
				...
				if (event.getButton() == ClickEvent.BUTTON_RIGHT) {
					...
					if (event.getClientX() > TableViewerCellSelectorUtil.this.table.getWidth()
						...
						TableViewerCellSelectorUtil.this.contextWindow
								.setPositionX((int) (event.getClientX() - TableViewerCellSelectorUtil.this.contextWindow.getWidth()));
					} else {
					...
					if (event.getClientY() > TableViewerCellSelectorUtil.this.table.getHeight()
						...
						TableViewerCellSelectorUtil.this.contextWindow
								.setPositionY((int) (event.getClientY() - TableViewerCellSelectorUtil.this.contextWindow.getHeight()));
					} else {
					...
					TableViewerCellSelectorUtil.this.cp.addListener(new ColorPicker.ColorChangeListener() {
						...
						public void colorChanged(final ColorChangeEvent event) {
							TableViewerCellSelectorUtil.this.cssClassName = TableViewerCellSelectorUtil.this
									.addColor(event.getColor().getRed(), event.getColor().getGreen(), event.getColor().getBlue());
							TableViewerCellSelectorUtil.this.applyColorToSelectedCells(TableViewerCellSelectorUtil.this.cssClassName,
									event.getColor().getRed(), event.getColor().getGreen(), event.getColor().getBlue());
							if (TableViewerCellSelectorUtil.this.source instanceof Window) {
								((Window) TableViewerCellSelectorUtil.this.source).getParent()
										.removeWindow(TableViewerCellSelectorUtil.this.contextWindow);
								TableViewerCellSelectorUtil.this.contextWindowDisplayed = false;
							} else {
								TableViewerCellSelectorUtil.this.table.getWindow()
										.removeWindow(TableViewerCellSelectorUtil.this.contextWindow);
								TableViewerCellSelectorUtil.this.contextWindowDisplayed = false;
							}
						}
					...
					final CellCoordinate currentCellCoordinate =
	...
	private void addCoordinatesSelectedByShift() {
		...
		final int indexOfCurrentColumn = this.columnHeaders.indexOf(this.currentColumn);
		final int indexOfPreviousColumn = this.columnHeaders.indexOf(this.previousColumn);
		final int indexOfCurrentRow = itemIds.indexOf(Integer.valueOf(this.currentRow));
		final int indexOfPreviousRow = itemIds.indexOf(Integer.valueOf(this.previousRow));
		...
		for (int x = minColumn; x <= maxColumn; x++) {
			for (int y = minRow; y <= maxRow; y++) {
				final CellCoordinate currentCellCoordinate = new CellCoordinate(this.columnHeaders.get(x), itemIds.get(y));
	...
	private String addColor(final int r, final int g, final int b) {
		final String className = String.valueOf(r) + String.valueOf(g) + String.valueOf(b);
		if (this.customCSSClassNames.indexOf(className) == -1) {
			...
			if (this.source instanceof Window) {
				...
				((AbstractOrderedLayout) this.source)
						.addComponent(new Label("<style> .v-table-cell-content-" + className + " { background: rgb(" + r + "," + g + "," + b
								+ "); border-bottom:1px solid #e0e0e0; } </style>", Label.CONTENT_XHTML));
			}
		}
		return className;
	}
	 ...
	 * @param className
	 */
	private void applyColorToSelectedCells(final String className, final int redValue, final int greenValue, final int blueValue) {
		for (int i = 0; i < this.highlightedCellCoordinates.size(); i++) {
			...
			if (!inColorAssigmentsList) {
				final CellCoordinateColorAssignment cellCoordinateColorAssigment = new CellCoordinateColorAssignment(
						this.highlightedCellCoordinates.get(i), className, redValue, greenValue, blueValue);
	...
	public XSSFColor getColor(final String itemId, final String propertyId) {
		for (int i = 0; i < this.cellCoordinateColorAssigments.size(); i++) {
			if (this.cellCoordinateColorAssigments.get(i).getCellCoordinate().getX().equals(propertyId)
				...
				return new XSSFColor(new java.awt.Color(this.cellCoordinateColorAssigments.get(i).getRedValue(),
						this.cellCoordinateColorAssigments.get(i).getGreenValue(),
						this.cellCoordinateColorAssigments.get(i).getBlueValue()));
			}
		}
		return null;
	}
