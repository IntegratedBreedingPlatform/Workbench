...
public class GroupRelativesQuery implements Query {
	...
	public Item constructItem() {
		...
		item.addItemProperty(GroupRelativesQuery.GID, new ObjectProperty<String>(""));
		item.addItemProperty(GroupRelativesQuery.PREFERRED_NAME, new ObjectProperty<String>(""));
		return item;
	}
	...
	public List<Item> loadItems(int start, int numOfRows) {
		...
		try {
			germplasms.addAll(this.pedigreeDataManager.getGroupRelatives(this.gid, start, numOfRows));
		} catch (MiddlewareQueryException ex) {
			GroupRelativesQuery.LOG.error("Error with getting group relatives for gid = " + this.gid + ": " + ex.getMessage());
			return new ArrayList<Item>();
		}

		for (Germplasm germplasm : germplasms) {
			PropertysetItem item = new PropertysetItem();
			item.addItemProperty(GroupRelativesQuery.GID, new ObjectProperty<String>(germplasm.getGid().toString()));
			if (germplasm.getPreferredName() != null) {
				item.addItemProperty(GroupRelativesQuery.PREFERRED_NAME, new ObjectProperty<String>(germplasm.getPreferredName().getNval()));
			} else {
				item.addItemProperty(GroupRelativesQuery.PREFERRED_NAME, new ObjectProperty<String>("-"));
			}
			items.add(item);
	...
	public int size() {
		...
		try {
			if (this.size == 0) {
				this.size = ((Long) this.pedigreeDataManager.countGroupRelatives(this.gid)).intValue();
			}
