...
import org.generationcp.ibpworkbench.germplasm.containers.ListsForGermplasmQuery;
import org.generationcp.ibpworkbench.germplasm.containers.ListsForGermplasmQueryFactory;
...
public class GermplasmListComponent extends VerticalLayout implements InitializingBean, InternationalizableComponent {
	...
	public GermplasmListComponent(final Integer gid, final boolean fromUrl) {
		this.gid = gid;
		this.fromUrl = fromUrl;
	}

	// Called by GermplasmListItemClickListener
	public void listItemClickAction(final ItemClickEvent event, final Integer listId) {
		final Window mainWindow = event.getComponent().getWindow();
		...
		if (tool == null) {
			listBrowserLink = new ExternalResource(WorkbenchAppPathResolver
					.getFullWebAddress(DefaultGermplasmStudyBrowserPath.LIST_BROWSER_LINK + this.gid, "?restartApplication"));
		} else {
		...
		final Window germplasmListWindow = new BaseSubWindow("Germplasm List Information - " + listId);

		final VerticalLayout layoutForList = new VerticalLayout();
	...
	private void initializeComponents() {
		final ListsForGermplasmQueryFactory factory =
				new ListsForGermplasmQueryFactory(this.germplasmListManager, this.gid, this.contextUtil.getCurrentProgramUUID());
		...
		if (container.size() > 0) {
			...
			this.listsTable.setColumnHeader(ListsForGermplasmQuery.GERMPLASMLIST_DATE, this.messageSource.getMessage(Message.DATE));
