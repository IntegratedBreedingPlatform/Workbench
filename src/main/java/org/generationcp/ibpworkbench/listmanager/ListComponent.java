...
import org.generationcp.ibpworkbench.listmanager.dialog.AssignCodesDialog;
import org.generationcp.ibpworkbench.listmanager.dialog.GermplasmGroupingComponent;
import org.generationcp.ibpworkbench.listmanager.dialog.ListManagerCopyToNewListDialog;
import org.generationcp.ibpworkbench.listmanager.listeners.GidLinkButtonClickListener;
import org.generationcp.ibpworkbench.listmanager.util.FillWith;
import org.generationcp.ibpworkbench.listmanager.util.ListCommonActionsUtil;
...
public class ListComponent extends VerticalLayout implements InitializingBean, InternationalizableComponent, BreedingManagerLayout,
			...
			transactionTemplate.execute(new TransactionCallbackWithoutResult() {
				...
				protected void doInTransactionWithoutResult(final TransactionStatus status) {
					...
					if (action.equals(ListComponent.this.messageSource.getMessage(Message.DELETE_SELECTED_ENTRIES))) {
						...
						// blurred/readonly
						for (final Entry<Object, Field> entry : itemMap.entrySet()) {
	...
	private void performListEntriesDeletion(final Map<Object, String> itemsToDelete) {
		try {
			...
			for (final Entry<Object, String> item : itemsToDelete.entrySet()) {
